...
try:
    ...
    from qe.demo_events import logger as __demo_events_logger
except ModuleNotFoundError:
    from demo_events import EventDTO, generate_random_events
    from demo_events import logger as __demo_events_logger
...
if __name__ == "__main__":
    ...
    events = generate_random_events(10_000_000, 3 * 30 * 24 * 3600)  # 10 million events within the last ~3 months
    logging.basicConfig(level=logging.WARNING, format='%(asctime)s - %(levelname)s - %(message)s')
    logger.setLevel(logging.DEBUG)
    __demo_events_logger.setLevel(logging.DEBUG)
